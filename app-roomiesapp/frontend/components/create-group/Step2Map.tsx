      ...
      <div className="w-full md:w-[420px] h-full flex flex-col bg-white border-r border-gray-200 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.05)]">
        ...
        <div className="p-6 border-b border-gray-100 bg-white shrink-0 z-30">
          ...
          <div className="relative w-full">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-ra-green pointer-events-none z-10" />
            
            <Input
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Ex: bares, restaurantes..."
              className="w-full pl-12 pr-14 py-7 bg-white border-gray-200 focus-visible:ring-2 focus-visible:ring-ra-green focus-visible:border-ra-green rounded-xl text-base shadow-inner transition-all"
            />

            {(isSearching || isAdding) && (
              <div className="absolute right-4 top-1/2 -translate-y-1/2">
                <Loader2 className="w-5 h-5 text-ra-green animate-spin" />
              </div>
            )}
                ...
                <ul className="max-h-64 overflow-y-auto divide-y divide-gray-100">
                  {suggestions.map((suggestion) => (
                    <li key={suggestion.placePrediction.placeId}>
                      ...
                      >
                        <MapPin className="w-5 h-5 text-gray-400 group-hover:text-ra-green shrink-0 mt-0.5 transition-colors" />
        ...
        <div className="flex-1 p-4 space-y-3 bg-white overflow-y-auto">
          {stops.length === 0 ? (
            ...
            stops.map((stop, index) => (
              <div key={stop.id} className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-2xl shadow-sm hover:border-ra-green/40 transition-all group">
                
                <div className="flex flex-col gap-1">
                  <button 
                    onClick={() => moveStop(index, "up")} 
                    disabled={index === 0} 
                    className="text-gray-300 hover:text-ra-green disabled:opacity-10 transition-colors"
                  >
                ...
                <button 
                  ...
                  className="p-2 text-gray-300 hover:text-ra-green transition-colors"
                >
